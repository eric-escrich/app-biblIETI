...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, InputTextModule, ButtonModule, PasswordModule, DialogModule, RouterLink, FormsModule, ReactiveFormsModule, DividerModule],
        ...
        if (this.loginForm.status === 'INVALID') {
            ...
            if (passwordControl && repeatPasswordControl && passwordControl.value !== repeatPasswordControl.value) {
                this._logService.logWarning(
                    ...
                    `Un bibliotecari ha intenta crear un usuari, però ha introduït incorrectamente la contraseña: ${this.errorMessage}`,
                    'CreacioUsuariComponent - onConfirm',
                );
